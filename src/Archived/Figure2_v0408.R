...
labelD <- eventLen_scatterD%>% 
  ...
  mutate(ypos = c(0.1,0.3,0.35,0.5)) %>% 
  mutate(xpos = c(rep(10000,2),rep(10^5*0.5,2)))
SegD <- labelD %>%
  select(label,
         x_end = xpos,x_start = eventLen,
         y_end = ypos,y_start = recall)
SegD$x_end[1:2] <- SegD$x_end[1:2]*1.5
SegD$y_end[1:2] <- SegD$y_end[1:2]+c(-0.1,0.1)*0.7
mode_shapes[1:4] <- c(0,1,2,4)
eventLen_scatterD$revSize = 1/(eventLen_scatterD$recall+0.4)
eventLen_scatter <-   ggscatter(eventLen_scatterD,
  ...
  geom_curve(
    ...
    arrow = arrow(ends = "last",length = unit(0.04, "npc")),
    color = 'black',
    size = 0.6)+
  geom_label(
    data = labelD,
...
event_eval <- plot_grid(upset_detect, eventLen_scatter,
                        ...
                        rel_widths=c(0.7,1))
extrafont::loadfonts()
p <- plot_grid(combined_eval, event_eval,ncol=1,rel_heights =c(2.1,1.2))
